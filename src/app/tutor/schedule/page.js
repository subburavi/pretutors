...
const MyLessonsPage = () => {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      try {
        const response = await axios.get(APIURL + 'tutor/lessons/' + user.id);
  ...
  const filteredLessons = filterLessonsByTab(lessons, activeTab).filter(lesson =>
    lesson.student_full_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <div className="flex items-center space-x-4 flex-1">
            <div className="relative">
              <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-md">
                {lesson.student_full_name.slice(0, 2)}
              </div>
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-white"></div>
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-2 mb-1">
                <h3 className="font-semibold text-gray-900 truncate">{lesson.student_full_name}</h3>
              </div>
